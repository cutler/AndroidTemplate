...
public class HttpURLConnectionUploadTask extends Thread implements TransferableTask {
	...
	public void upload(UploadFile file, Map<String, String> params) {
		...
		try {
			...
			conn.setRequestProperty("Charsert", HttpCaller.ENCODE_CHARSET_NAME);
			...
			if (params != null && params.size() > 0) {
				...
				for (Map.Entry<String, String> entry : params.entrySet()) {
					...
					sb.append("Content-Type: text/plain; charset=" + HttpCaller.ENCODE_CHARSET_NAME + LINEND);
			...
			if (file != null) {
				...
				sb1.append("Content-Type: application/octet-stream; charset=" + HttpCaller.ENCODE_CHARSET_NAME + LINEND);
				sb1.append(LINEND);
				...
				outputStream.flush();
				// 获取返回数据
				result = (String) HttpCaller.executeRequest(conn, String.class);
			}
		}  catch (Exception e) {
			error = e;
		} finally {
	...
	private void publishProgress(int count) {
		...
		if(count == -1 || millis - NOTIFY_PROGRESS_INTERVAL >= previousPublishTime){
			...
			// 上传进度改变。 
			onUploadStateChanged(TransferObserver.TransferState.PROGRESS);
		}
	}
	...
	private void onPostExecute(String result) {
		if (deleted || paused || error != null) {
			if (error != null) {
				onUploadStateChanged(TransferObserver.TransferState.ERROR);
			}
			if (paused) {
				onUploadStateChanged(TransferObserver.TransferState.PAUSED);
			} else if (deleted) {
				onUploadStateChanged(TransferObserver.TransferState.DELETED);
			}
			return;
		}
		this.result = result;
		onUploadStateChanged(TransferObserver.TransferState.FINISHED);
	}

	/*
     * 当上传状态改变时，回调此方法通知Uploader。
     */
	private void onUploadStateChanged(final TransferObserver.TransferState state) {
		if (listener != null) {
				...
				public void run() {
					if (listener != null) {
						...
						if (fileState != null) {
							...
							UploadListDAO.getInstance().doCreateOrUpdate(uploadFile);
						}
					}
				}
			}).sendToTarget();
