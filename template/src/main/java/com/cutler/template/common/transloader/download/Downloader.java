...
public class Downloader extends AbstractLoader {
	...
	protected void onPreStartLoader() {
		try {
			List<DownloadFile> list = DownloadListDAO.getInstance(Template.getApplication()).findAllDownloadFile();
	...
	protected void onTransferableFileAddedTask(TransferableFile file) {
		...
		// 加入数据库
		DownloadListDAO.getInstance(Template.getApplication()).doCreateOrUpdate((DownloadFile) file);
	}
	...
	public boolean deleteLocalFileByUrl(String url){
		try {
			...
			File file = new File(StorageUtils.getDiskCacheDir(Template.getApplication(), InnerConfig.CACHE_DOWNLOAD), fileName);
			return file.delete();
		} catch (Exception e) {
			e.printStackTrace();
		}
