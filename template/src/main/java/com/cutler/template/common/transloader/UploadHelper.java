...
import com.cutler.template.util.base.NotificationUtil;
...
public class UploadHelper {
	...
	public static void uploadFileWithNotify(final Context context, final UploadFile uploadFile, final UploadCallback dCallback){
		...
		TransferObserver observer = new TransferObserver() {
			NotificationUtil.OnProgressListener callback;
			@Override
			public void onTransferDataChanged(TransferState state, Map<String, Object> params) {
				...
				switch(state){
				case ADDED:
					callback = NotificationUtil.showProgressNotify(context,
				...
				case PROGRESS:
					...
					break;
				case FINISHED:
					NotificationUtil.cancelNotify(context, notifyIds.get(uploadFile.getLocalPath()));
