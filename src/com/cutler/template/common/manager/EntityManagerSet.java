 ...
 * 本类用来管理一组EntityManager，使用Lru缓存的方式。 且每个EntityManager都必须是InSetEntityManager的子类。
 * 但是由于Lru的特点，会导致从Lru缓存中移除的InSetEntityManager实例，仍然拥有自己的观察者，因此又使用了HashMap来缓存那些从Lru缓存中被移除，但是仍有有观察者的InSetEntityManager。
 */
