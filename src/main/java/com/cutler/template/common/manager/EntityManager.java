...
/**
 * 程序中经常存在一些全局的对象，比如代表当前登录用户的User对象，EntityManager就是用来管理这些对象。
 * 1、 EntityManager管理对象的请求、缓存、失效时间、自动更新。
 * 2、 当被EntityManager管理的对象的信息发生改变时，EntityManager会通过回调的方式同时所有观察者。
 * @author cutler
 */
public abstract class EntityManager<T> {
	...
	protected void load(final ModelCallback callback, final Activity act, final String msg) {
		...
		if (data != null) {
			// 从本地加载成功后，进一步验证数据是否过期了。  若expireTime == 0 则视为数据不会失效。
		...
		loadFromNetWork(new ModelCallback() {
			public void callback(boolean success, Object... args) {
	 ...
	 * 注意：此方法只能在ui线程中被调用。
	 */
	final public void loadFromNetWork(final ModelCallback callback){
