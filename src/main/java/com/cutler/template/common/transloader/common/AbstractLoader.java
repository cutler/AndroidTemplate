...
public abstract class AbstractLoader extends Thread {
	...
	// 主线程中的Handler。
	protected Handler mHandler = new Handler(MainApplication.getInstance().getMainLooper());
	...
	public AbstractLoader(int maxTaskCount, int maxDownloadThreadCount) {
		this();
		if (maxTaskCount >= 1) {
			this.maxTaskCount = maxTaskCount;
		}
		if (maxDownloadThreadCount >= 1) {
    ...
    public void run() {
        while (isRunning) {
        	TransferableTask task = mWatingTaskQueue.poll();
			if (task != null) {
	...
	public boolean hasTransferableFile(TransferableFile file) {
		...
		for (int i = 0; i < mWatingTaskQueue.size(); i++) {
			TransferableTask task = mWatingTaskQueue.get(i);
			if (task.getTransferableFile().getUniqueNumber().equals(file.getUniqueNumber())) {
                return true;
            }
		}
	...
	public synchronized void deleteTransferableFile(TransferableFile file) {
        ...
        for (TransferableTask task : mDoingTaskList) {
        	if (task != null && task.getTransferableFile().getUniqueNumber().equals(uniqueNumber)) {
                task.stopTask();
                if (file.isDeleteCacheFile()) {
                	onDeleteCacheFile(task);
                }
                findTask = task;
                break;
            }
	 ...
	 * 当需要删除本地缓存文件时，调用此方法。
	 */
	protected abstract void onDeleteCacheFile(TransferableTask task);
	...
	protected class TaskQueue {
        ...
        public TransferableTask poll() {
            ...
            while (isRunning && (mDoingTaskList.size() >= maxDownloadThreadCount
	...
	public List<TransferableTask> getmPausingTaskList() {
		return mPausingTaskList;
	}

	public TaskQueue getmWatingTaskQueue() {
		return mWatingTaskQueue;
	}
     ...
     * 在主线程中，通知所有观察者数据已经改变。 
     */
    public void notifyObservers(final TransferState state, final Map<String,Object> params){
    	AbstractManager.getMainHandler().post(new Runnable() {
			public void run() {
				if (observers != null) {
					// 通知所有观察者，数据已经改变。
