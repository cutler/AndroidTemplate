...
import com.cutler.template.media.R;
...
public class VideoPlayer extends LinearLayout {
    ...
    private void initRootView() {
        LayoutInflater.from(getContext()).inflate(R.layout.tp_view_video_player, this);
            ...
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                if (getContext() instanceof Activity) {
                    ...
                    if (isChecked) {
                        ...
                        // 启动全屏模式，主要是把状态栏给隐藏了。
                        VideoUtil.openFullScreen(act);
                        realWidth = VideoUtil.getScreenWidth(act);
                        realHeight = VideoUtil.getScreenHeight(act);
                    } else {
                        act.setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_PORTRAIT);
                        VideoUtil.closeFullScreen(act);
                        realWidth = initWidth;
                        realHeight = initHeight;
                    }
                    // 让Activity继续接受手机横竖屏事件。
